...
if __name__ == '__main__':
    ...
    n_hidden_neurons = [[(1,len(t)),  (1,1,len(t)),   (1,1,1,len(t))   ],
                       ...
                       [ (50,len(t)), (50,50,len(t)), (50,50,50,len(t))],
                       [ (100,len(t)), (100,100,len(t)), (100,100,100,len(t))]]
    nlayers = len(n_hidden_neurons)
    num_iter = 200
    eta = 4e-3#[8e-4, 1e-3, 4e-3, 8e-3, 1e-2]
    lmbda = torch.tensor(1e-4)  #torch.tensor(np.logspace(-4,-1,4) )
    lneurons = np.array([1, 5, 10, 20, 50, 100])
    llayers = np.array([1,2,3])
    mses = torch.zeros((len(lneurons), len(llayers)))
    
    
    for i in range(len(n_hidden_neurons)):
        for j in range(len(n_hidden_neurons[0])):
            num_hidden_neurons = n_hidden_neurons[i][j]
    ...
    plot_heatmap(llayers,lneurons,mses, title = 'Test MSE', type_axis = 'int',
             xlabel="Layers", ylabel="Neurons", cbar_label = 'MSE', vmin = np.min(mses), vmax=np.max(mses),
             store = True, store_dir = os.getcwd() + "/../plots/test_mses_net_arch_pde_solver_100iter.png")
